...
public class LevelUp {
  private void addNewCommEdge(HashMap<Integer, HashMap<Integer, Double>> newGraphInfo, int own_comm, int neigh_comm, double w, boolean should_divide) {
    ...
    if (should_divide) // bug fix: after level 1 of louvain iterations, the self loop community edges won't appear more than once, so we must not divide those by 2. 
  ...
  private void printNewGraph(HashMap<Integer, HashMap<Integer, Double>> newGraphInfo, String outputFile) throws IOException {
  ...
  private HashMap<Integer, HashMap<Integer, Double>> orderAndMapIds(HashMap<Integer, HashMap<Integer, Double>> newGraphInfo, HashMap<Integer, Integer> idMap) throws IOException {
    HashMap<Integer, HashMap<Integer, Double>> orderedGraphInfo = new HashMap<Integer, HashMap<Integer, Double>>();
    int idIncrementer = -1;
    for (int source : newGraphInfo.keySet()) {
      int newSource = 0;
  ...
  public int prepareForNextLevel(Vertex vertices[], Community []communities, String outputFile, HashMap<Integer, Integer> idMapper, HashMap<Integer, Integer> vertices_to_comm) throws IOException {
    HashMap<Integer, HashMap<Integer, Double>> newGraphInfo = new HashMap<Integer, HashMap<Integer, Double>>();
    ...
    for (Vertex v : vertices) {
      ...
      for (Neighbor n : v.neighbors) {
        ...
        addNewCommEdge(newGraphInfo, own_comm, neigh_comm, n.edge_weight, (v.id != n.neighbour_label));
      }
    }
