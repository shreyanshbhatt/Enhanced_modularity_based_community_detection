...
class Indexer {
  ...
  public void addDepth(String inputConcept, HashMap<String, Tuple2<HashSet<String>, Integer>> index) {
    ...
    while(!queue.isEmpty()) {
      ...
      for (String child : children) {
        ...
        index.put(child, new Tuple2<HashSet<String>, Integer>(conceptParentIndex.get(child), level));
  ...
  private void addForwardParent(String sub, String pred, String obj, HashMap<String, HashSet<String>> conceptParentIndex) {
    if (pred.equals("<http://dbpedia.org/ontology/wikiPageWikiLink>")) {
      ...
      if (conceptParentIndex.containsKey(obj)) {
        // we have already seen this object and we will be adding to its parents.
      ...
      // article mentioned as the same level as article we will include the artcile's parents
      // as the parents.
      if (conceptParentIndex.get(sub) == null) {
        // This is a very special case. our root itself had some articles listed in itself so we won't be
    ...
    } else if (pred.equals("<http://purl.org/dc/terms/subject>") || pred.equals("<http://www.w3.org/2004/02/skos/core#broader>")) {
  ...
  private void addReverseParent(String sub, String pred, String obj, HashMap<String, HashSet<String>> reverseParentIndex) {
    if (pred.equals("<http://dbpedia.org/ontology/wikiPageWikiLink>")) {
...
class AuthorComparator {
  ...
  public void readLines(String fileName, ArrayList<ArrayList<Tuple2<String, Integer>>> authorDesc) throws Exception {
  ...
  private Tuple2<Double, Double> compute_wighted_jaccard(ArrayList<HashMap<String, Double>> author1, 
      ...
      for (HashMap<String, Double> author2Matched : author2) {
        for (String concept : author1Matched.keySet()) {
          if (author2Matched.containsKey(concept)) {
            numer += getJaccardWeightDistane(author1Matched.get(concept), author2Matched.get(concept));
          }
        }
      }
    }
    ...
    for (String concept1 : author1.get(0).keySet()) {
      for (String concept2 : author2.get(0).keySet()) {
        denom += getJaccardWeightDistane(author1.get(0).get(concept1), author2.get(0).get(concept2));
      }
    }
  ...
  private void getAuthorMultipleLevelList(ArrayList<HashMap<String, Double>> newListHierarchy, ArrayList<Tuple2<String, Integer>> existingList,
    ...
    for (Tuple2<String, Integer> existing : existingList) {
      ...
      if (actualConceptsList.containsKey(existing._1)) {
        // The most specific concept defining this author already exists in the hierachy.
      ...
      // now start parents
      for (int parentLevel = 2; parentLevel < ConceptIndexerAuthorScorer.maxParentLevel; parentLevel++) {
        ...
        while (!checkParents.isEmpty()) {
          ...
          if (index.get(checkParent)._1 == null) {
            ...
            if (!should_allow) {
              System.out.println(checkParent+" caused error since it wasn't found while it should in index");
              System.exit(1);
            }
            continue;
          }
          for (String par1 : index.get(checkParent)._1) {
            double score_for_this = getConceptScore(parentLevel, index.get(par1)._2, no_of_mentions);
            if (thisLevelList.containsKey(par1)) {
              // if a concept for this author already exist in parent hierachy and we find it again.
  ...
  private Tuple2<Double, Double> getAreaScore(HashMap<String, Tuple2<HashSet<String>, Integer>> index,
      ArrayList<Tuple2<String, Integer>> author1, ArrayList<Tuple2<String, Integer>> author2, String aid1, String aid2, int indexNum) {
  ...
  private ArrayList<Double> getAuthorInteraction(ArrayList<HashMap<String, Tuple2<HashSet<String>, Integer>>> index,
      ArrayList<Tuple2<String, Integer>> author1, ArrayList<Tuple2<String, Integer>> author2, String aid1, String aid2) {
    ...
    for (int indexId = 0; indexId < index.size(); indexId++) {
      Tuple2<Double, Double> numer_denom = getAreaScore(index.get(indexId), author1, author2, aid1, aid2, indexId);
  ...
  public void computeAuthorDistance(String authorEdgeFile, 
    ...
    for (String line : lines) {
      ...
      ArrayList<Double> four_area_interaction_scores = getAuthorInteraction(index, authorDesc.get(author1), authorDesc.get(author2), ""+author1, ""+author2);
      bw.write(""+author1);
  ...
  private void checkInc(double []vals) {
  ...
  private void normalizeArray(double []vals) {
    ...
    if (!Double.isNaN(mean) && Double.isNaN(stdDev) && stdDev != 0.0) {
      for (int i = 0; i < vals.length; i++) {
        double zVal = (vals[i] - mean)/stdDev;
        vals[i] = zVal;
      }
    }
    for (int i = 0; i < vals.length; i++) {
      vals[i] += ConceptIndexerAuthorScorer.meanShift;
    }
  }
  
  private void normalize(double [][]vals) {
    for (int i = 0; i < vals.length; i++) {
      normalizeArray(vals[i]);
    }
  }
  
  public void normalizeNewEdgeWeights(String fileName, int numFeatures) throws IOException {
    List<String> lines = Files.readAllLines(Paths.get(fileName));
    double [][] existingVals = new double[numFeatures][lines.size()];
    for (int lineno = 0; lineno < lines.size(); lineno++) {
      String splits[] = lines.get(lineno).split(" ");
      for (int i = 1; i < splits.length - 1; i++) {
        existingVals[i-1][lineno] = Double.parseDouble(splits[i]);
      }
    }
    normalize(existingVals);
    ...
    for (int lineno = 0; lineno < lines.size(); lineno++) {
      ...
      bw.write(""+src);
      for (int i = 1; i < splits.length - 1; i++) {
        bw.write(" "+existingVals[i - 1][lineno]);
      }
      bw.write(" "+target);
      bw.newLine();
    }
...
public class ConceptIndexerAuthorScorer {
  ...
  private static String []files = {
      "gplus_domains/institutions.ttl",
      "gplus_domains/occupations.ttl",
      "gplus_domains/place.ttl",
      "gplus_domains/university.ttl"};
  public static int maxParentLevel = 4;
  public static double meanShift = 8.0;
  public static void main(String []args) throws Exception {    
    ArrayList<HashMap<String, Tuple2<HashSet<String>, Integer>>> index = new ArrayList<HashMap<String, Tuple2<HashSet<String>, Integer>>>();
    for (int i = 0; i < domains.length; i++) {
      HashMap<String, Tuple2<HashSet<String>, Integer>> index_temp = new HashMap<String, Tuple2<HashSet<String>, Integer>>();
    ...
    for (final File fileEntry : new File("./gplus_all/").listFiles()) {
      if (!fileEntry.getName().endsWith(".edges")) {
        continue;
      }
      String authorId = fileEntry.getName().split("\\.")[0];
      AuthorComparator ap = new AuthorComparator();
      ArrayList<ArrayList<Tuple2<String, Integer>>> authorDescriptions = new ArrayList<ArrayList<Tuple2<String, Integer>>>();
      ap.readLines("./gplus_all/"+authorId+".featuresdb", authorDescriptions);
      ap.computeAuthorDistance(fileEntry.getAbsolutePath(), index, authorDescriptions);
      ap.normalizeNewEdgeWeights(fileEntry.getAbsolutePath()+"_annotated", 4);
    }
  }
}
