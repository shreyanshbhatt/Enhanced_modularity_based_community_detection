...
class JaccardC {
  ...
  private void readFileInDS(String fileName, HashMap<Integer, ArrayList<String>> ds) throws Exception {
  ...
  public double computeFMeasure(String groundTruth, String detected) throws Exception {
    ...
    for (int dt : dtCNMembership.keySet()) {
      ...
      total_f1_score += (double)((double)(max_f1_score)/(double)(2.0 * (double)dtCNMembership.size()));
    }
...
public class CommunityJaccard {
  ...
  public double getJaccard(int id, String detected) throws Exception {
    ...
    double one_way = fmc.computeFMeasure(detected, "facebook/newCircles"+id);
    double other = fmc.computeFMeasure("facebook/newCircles"+id, detected);
